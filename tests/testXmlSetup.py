...
class TestXmlFlagTypesRoundTrip(unittest.TestCase):
    """Round-trips the remaining flag types (datetime, file, and choice with
    its FlagChoice options) and asserts capture behavior survives."""

    LEVEL_NUMBER = 10
    CORP_NAME = "FlagTypes Corp"
    BOX_NAME = "FlagTypesBox"
    FILE_DATA = "the-file-bytes"
    DATETIME = "2020-01-02 03:04:05"
    CHOICES = ("OptionA", "OptionB", "OptionC")
    CORRECT_CHOICE = "OptionB"
    ...
    def setUp(self):
        ...
        self.box = Box(corporation_id=self.corp.id, game_level_id=self.level.id)
        self.box.name = self.BOX_NAME
        self.box.description = "flag types"
        self.corp.boxes.append(self.box)
        dbsession.add(self.box)
        dbsession.commit()

        # Flag.create_flag() builds but does not persist; add each explicitly.
        self.datetime_flag = Flag.create_flag(
            _type=FLAG_DATETIME,
            box=self.box,
            name="RT Datetime",
            raw_token=self.DATETIME,
        ...
        self.file_flag = Flag.create_flag(
            ...
            box=self.box,
            name="RT File",
            raw_token=encode(self.FILE_DATA),
            description="file",
            value=20,
        )
        ...
        for option in self.CHOICES:
            choice = FlagChoice(flag_id=self.choice_flag.id)
    ...
    def _serialize_and_reimport(self):
        root = ET.Element("rootthebox")
        root.set("api", "1")
        levels_elem = ET.SubElement(root, "gamelevels")
        self.level.to_xml(levels_elem)
        ...
        try:
            ...
            os.unlink(tmp.name)
        return Corporation.by_name(self.CORP_NAME).boxes[0]
    ...
    def test_flag_types_round_trip(self):
        ...
        self.assertFalse(flags["RT Datetime"].capture("1999-09-09 09:09:09"))

        # file: token is the sha1 digest; the same bytes still capture.
        self.assertTrue(flags["RT File"].capture(encode(self.FILE_DATA)))
        self.assertFalse(flags["RT File"].capture(encode("other-bytes")))
